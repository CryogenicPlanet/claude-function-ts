...
export const Err = <T>(err: T) => new ErrImpl(err);
...
export namespace Result {
  ...
  export function all<T extends Result<any, any>[]>(
    ...
    return new OkImpl(okResult as ResultOkTypes<T>);
  }
  ...
  export function any<T extends Result<any, any>[]>(
    ...
    // it must be a Err
    return new ErrImpl(errResult as ResultErrTypes<T>);
  }
  ...
  export function wrapAsync<T, E = unknown>(
    ...
    try {
      return op()
        .then((val) => new OkImpl(val))
        .catch((e) => new ErrImpl(e));
    } catch (e) {
      return Promise.resolve(new ErrImpl(e as E));
    }
  }
